...
- Check server is running: `pm2 status` or `npm run dev` output
- Verify `VITE_JAM_SERVER_URL` matches server URL
- Check CORS: `CLIENT_URL` in server `.env`

**Audio not syncing**:
- Open browser console on both clients
- Look for "Drift:" logs in SyncedAudioPlayer
- Verify both clients in same room (check room ID in header)
- Check WebSocket connection (Network tab → WS filter)

**Navidrome authentication fails**:
- Verify Navidrome URL is accessible
- Check credentials are correct
- Inspect Network tab for `/rest/ping` response
